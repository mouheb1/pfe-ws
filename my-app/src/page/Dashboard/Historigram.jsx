    ...
    const handleMessageTwo = (dataSTR) => {  
        try {
            ...
            if (dataJson.mode ==  "data") { 
                if (  !dataJson.hasOwnProperty('content')|| !dataJson.content.hasOwnProperty('nombre_pieces')) {console.error('Cant found content or nombre_pieces'); return ;}
                    ...
                    if (index !== -1) {
                        ...
                        updatedArray[index].totalPiecesPalatize  = updatedArray[index].totalPiecesPalatize + parseFloat( dataJson.content.nombre_pieces);
                        newdata =  updatedArray;
                    } else {
                        newdata =  [...  state.dataColumns, {name:   dataJson.content.reference, totalPiecesPalatize:   parseFloat( dataJson.content.nombre_pieces)}];
                    }
